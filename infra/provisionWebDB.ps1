...
if ($numRows.Column1 -eq 0) {
    ...
    & "C:\Program Files\Microsoft SQL Server\Client SDK\ODBC\170\Tools\Binn\bcp" $dbName+".dbo.Courses" in $releaseDirectory\_LearnDB-ASP.NETCore-CI\drop\courses.csv -S $($servername).database.windows.net -U "$adminLogin" -P "$adminPassword" -q -c -t "," -F 2
}
